...
CardClass = {}
 ...
 function CardClass:new(xPos, yPos, Suite, Rank, Color, Flipped)
   local cardClass = {}
    ...
    cardClass.spade = love.graphics.newImage("assets/spade.jpeg")
    cardClass.heart = love.graphics.newImage("assets/heart.jpeg")
    cardClass.club = love.graphics.newImage("assets/club.jpeg")
    cardClass.diamond = love.graphics.newImage("assets/diamond.jpeg")
 ...
 function CardClass:draw()
    ...
    if self.flipped == false then
      ...
      if self.suite == "S" then
        love.graphics.draw(self.spade, self.position.x + Width/2-11, self.position.y + Height/2 - 12, 0, card_scale, card_scale)
      elseif self.suite == "H" then
        love.graphics.draw(self.heart, self.position.x + Width/2-8, self.position.y + Height/2 - 5, 0, card_scale, card_scale)
      elseif self.suite == "C" then
        love.graphics.draw(self.club, self.position.x + Width/2-11, self.position.y + Height/2 - 12, 0, card_scale, card_scale)
      else
        love.graphics.draw(self.diamond, self.position.x + Width/2-11, self.position.y + Height/2 - 12, 0, card_scale*0.5, card_scale*0.5)
      end
    else
  ...
  if isMouseOver and love.mouse.isDown(1) and not grabber.holding and self.flipped == false then
